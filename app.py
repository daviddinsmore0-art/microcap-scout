...
def init_db():
    # STRICTLY EXPANDED ERROR HANDLING TO PREVENT SYNTAX ERRORS
    try:
        conn = get_connection()
        cursor = conn.cursor()
...
def calculate_risk(row, ai_score=None):
    ...
    if float(row.get('volatility') or 0) > 3.0: s += 10
...
# UI - FLATTENED HTML TO PREVENT RAW CODE
...
with c1: st.markdown(f"<div style='font-size:18px; font-weight:800; color:white; margin-top:10px;'>{get_greeting(display_name)}</div>", unsafe_allow_html=True)
with c2:

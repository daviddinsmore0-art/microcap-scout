...
def init_db():
    # Attempt to create tables if they don't exist to prevent the Crash
    try:
        conn = get_connection()
        cursor = conn.cursor()
        
        # Ensure daily_briefing exists (Fixes the Screenshot 3 Crash)
        cursor.execute("""
        ...
        # Ensure stock_cache exists (Fixes the Screenshot 2 Schema)
        cursor.execute("""
        ...
        # Don't stop app execution here, just log it
        print(f"DB Init Error: {e}")

# --- Helper: Fix the "999d" Error ---
def parse_smart_date(date_str):
    """
    Parses 'Feb 12' or '2026-02-12' and returns days remaining.
    Fixes the '999d' issue seen in Screenshot 1.
    """
    if not date_str or str(date_str).lower() in ['n/a', 'none', '', '999']:
        return 999
    
    try:
        # Try "Feb 12" format (seen in DB screenshot)
        now = datetime.now()
        try:
            target = datetime.strptime(f"{date_str} {now.year}", "%b %d %Y")
            if target < now: # If date passed, assume next year
...
def calculate_risk(row):
    ...
    # Use pre_post_price if available (from Screenshot 2)
    price = float(row.get('pre_post_price') or row.get('current_price') or 0)
    if price < 5: s += 5 # Penny stocks are riskier
...
def get_watchlist_candidates():
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    # Using columns visible in your DB screenshot
...
if tab == "home":
    # 1. SAFE BRIEFING FETCH (Fixes Screenshot 3 Crash)
    try:
        ...
        # Silently fail if table missing, don't crash app
        pass
    ...
    if not portfolio:
        ...
        if valid_rows:
            ...
            # 2. EARNINGS FIX (Fixes Screenshot 1 "999d")
            earnings_candidates = []
            for r in valid_rows:
                # Use the new helper to parse "Feb 12" from DB
                d_val = parse_smart_date(r.get('next_earnings'))
                if d_val < 365: # Only count if it's within a year
    ...
    # Portfolio render logic matches your original

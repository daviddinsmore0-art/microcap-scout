...
# ******************************************* if 'initialized' not in st.session_state:
    ...
    st.session_state.update({
        ...
        'spy_last_fetch': datetime.min, 'banner_msg': None,
        'meta_cache': {} # Added for stability
    })

# --- STABILITY SHIELD (Added to YOUR Code) ---
def safe_fetch(ticker_obj, method, timeout=0.8):
    """Prevents the '1ST' Server Crash by killing hung requests."""
...
def get_relative_time(date_str):
    try:
        ...
        now = datetime.now(dt.tzinfo); diff = now - dt; seconds = diff.total_seconds()
...
def process_ai_batch(items_to_process, key):
    try:
        ...
        prompt = """Analyze these financial headlines. Return JSON: {'items': [{'ticker': '...', 'sentiment': 'BULL/BEAR/NEUTRAL', 'summary': '...', 'link': '...', 'time': '...'}]}"""
...
NAMES = {"TD.TO": "TD Bank", "BN.TO": "Brookfield", "CCO.TO": "Cameco", "IVN.TO": "Ivanhoe Mines", "HIVE": "Hive Digital", "SPY": "S&P 500 ETF", "BAER": "Baer Tech", "RERE": "ReRe Inc", "IMNN": "Imunon", "TX": "Ternium"} 
...
with c2: 
    if st.button("🔊 Test"): log_alert("Test Signal to Discord!", sound=True) 
...
if st.session_state.alert_log:
    with st.sidebar.expander("Recent Activity", expanded=True):
        for a in st.session_state.alert_log[:5]: st.caption(a) 
...
st.sidebar.selectbox("Price Target Asset", sorted(ALL_T), key="a_tick_input", on_change=update_params) 
...
# --- 4. DATA ENGINE (MODIFIED FOR STABILITY) ---
...
def get_pro_data(s):
    try:
        ...
        if h is None or h.empty:
            h = safe_fetch(tk, "history_5d") # Fallback
        if h is None or h.empty: return None
        
        p_live = h['Close'].iloc[-1]
        
        # 2. Historical Data (Medium)
        ...
        is_market_open = (now.weekday() < 5) and ((now.hour > 9 or (now.hour == 9 and now.minute >= 30)) and (now.hour < 16))
        is_tsx = any(x in s for x in ['.TO', '.V', '.CN']) 

        if is_market_open: display_price = p_live; display_pct = ((p_live - prev_close) / prev_close) * 100
        else: display_price = hard_close; display_pct = ((hard_close - prev_close) / prev_close) * 100 

        market_state = "REG"; ext_price = None; ext_pct = 0.0
        ...
        # 4. Charting
        spy = get_spy_data()
        chart_data = h['Close'].tail(78).reset_index(); chart_data.columns = ['T', 'Stock']; chart_data['Idx'] = range(len(chart_data)) 
        ...
        if spy is not None and len(spy) > 0:
            s_slice = spy.tail(len(chart_data)); spy_start = s_slice.iloc[0] if s_slice.iloc[0] != 0 else 1
            s_norm = ((s_slice - spy_start) / spy_start) * 100
            chart_data['SPY'] = s_norm.values if len(s_norm) == len(chart_data) else 0 

        # 5. Metadata (Slowest - Protected)
        now_ts = time.time()
        if s not in st.session_state['meta_cache'] or (now_ts - st.session_state['meta_cache'][s][1] > 3600):
            ...
            st.session_state['meta_cache'][s] = ({"rat": rat, "earn": earn}, now_ts)
        ...
        if (datetime.now() - last_alert).total_seconds() > 300:
            if trend == "BULL" and rsi < 35 and vol_ratio > 1.2:
                log_alert(f"⚡ **PERFECT STORM:** {s}\n- RSI: {rsi:.0f}\n- Trend: Bullish Dip"); st.session_state['storm_cooldown'][s] = datetime.now()
            elif trend == "BEAR" and rsi > 65 and vol_ratio > 1.2:
                log_alert(f"🐻 **DEATH BEAR:** {s}\n- RSI: {rsi:.0f}\n- Trend: Bearish Rejection"); st.session_state['storm_cooldown'][s] = datetime.now() 

        return {
            "p": display_price, "d": display_pct, "rsi": rsi, "tr": trend, "vol": vol_ratio,
            "chart": chart_data, "ai": ai_bias, "rat": meta['rat'], "earn": meta['earn'],
...
with h1:
    img_html = f'<img src="data:image/png;base64,{get_base64_image(LOGO_PATH)}" style="max-height:80px;">' if get_base64_image(LOGO_PATH) else "<h1>⚡ Penny Pulse</h1>"
...
with h2:
    ...
    <div style="font-family:'Helvetica', sans-serif; display:flex; justify-content:flex-end; align-items:center; height:100%; padding-right:10px;">
        <div style="background:#1E1E1E; border:1px solid #333; border-radius:8px; padding:10px 15px; display:flex; align-items:center;">
            <div style="font-size:32px; font-weight:700; color:#FF4B4B; font-family:'Courier New', monospace;"><span id="timer">--</span><span style="font-size:14px;">s</span></div>
        </div>
    </div>
...
def draw_pro_card(t, port_data=None):
    ...
    if d:
        ...
        if d['state'] != "REG" and d['ext_p']:
            ...
            st.markdown(f"<div style='text-align:right; margin-top:-8px; margin-bottom:8px;'><span style='color:{ext_col}; font-size:14px; font-weight:bold;'>POST: ${d['ext_p']:,.2f} ({ext_sign}{d['ext_d']:.2f}%)</span></div>", unsafe_allow_html=True) 

        if port_data:
            qty = port_data['q']; entry = port_data['e']
            val = d['p'] * qty; profit = val - (entry * qty)
            p_col = "#4caf50" if profit >= 0 else "#ff4b4b"
            st.markdown(f"<div style='background-color:black; color:white; border-left:4px solid {p_col}; padding:8px; margin-bottom:10px; font-weight:bold;'>Qty: {qty} | Avg: ${entry} | Gain: <span style='color:{p_col};'>${profit:,.2f}</span></div>", unsafe_allow_html=True) 
        ...
        else: st.altair_chart(l1.properties(height=60), use_container_width=True)
        
        st.caption("INTRADAY vs SPY (Orange/Dotted)")
        
        pct = max(0, min(100, ((d['p'] - d['l']) / (d['h'] - d['l']) * 100 if d['h'] > d['l'] else 50)))
        st.markdown(f"""<div style="font-size:10px;color:#888;">Day Range</div><div style="width:100%;height:8px;background:linear-gradient(90deg, #ff4b4b, #ffff00, #4caf50);border-radius:4px;position:relative;margin-bottom:15px;"><div style="position:absolute;left:{pct}%;top:-2px;width:3px;height:12px;background:white;border:1px solid #333;"></div></div>""", unsafe_allow_html=True) 
        ...
        <div style="width:100%;height:8px;background:#333;border-radius:4px;overflow:hidden;margin-bottom:20px;"><div style="width:{rsi_pct}%;height:100%;background:{'#ff4b4b' if d['rsi']>70 else '#4caf50'};"></div></div>
        """, unsafe_allow_html=True)
        st.divider() 
...
with t2:
    ...
    with c1: st.markdown(f"""<div style="background:#1E1E1E;border:1px solid #333;border-radius:8px;padding:15px;text-align:center;"><div style="color:#888;font-size:12px;font-weight:bold;">NET LIQUIDITY</div><div style="font-size:24px;font-weight:900;color:white;">${total_val:,.2f}</div></div>""", unsafe_allow_html=True)
    with c2: st.markdown(f"""<div style="background:#1E1E1E;border:1px solid #333;border-radius:8px;padding:15px;text-align:center;"><div style="color:#888;font-size:12px;font-weight:bold;">DAY PROFIT</div><div style="font-size:24px;font-weight:900;color:{'#4caf50' if day_pl>=0 else '#ff4b4b'};">${day_pl:,.2f}</div></div>""", unsafe_allow_html=True)
    with c3: st.markdown(f"""<div style="background:#1E1E1E;border:1px solid #333;border-radius:8px;padding:15px;text-align:center;"><div style="color:#888;font-size:12px;font-weight:bold;">TOTAL RETURN</div><div style="font-size:24px;font-weight:900;color:{'#4caf50' if tpl>=0 else '#ff4b4b'};">${tpl:,.2f} ({total_roi:+.1f}%)</div></div>""", unsafe_allow_html=True)
...
with t3:
    FEEDS = ["https://finance.yahoo.com/news/rssindex", "https://search.cnbc.com/rs/search/combinedcms/view.xml?partnerId=wrss01&id=10000664", "http://feeds.marketwatch.com/marketwatch/topstories"]
                    ...
                    try:
                        ...
                        if r.status_code == 200:
                            ...
                            for item in root.findall('.//item'):
                                raw_items.append({"title": item.find('title').text, "link": item.find('link').text, "time": get_relative_time(item.find('pubDate').text)})
                                if len(raw_items) > 30: break
                    except: continue
                
                if raw_items:
                    st.session_state['raw_news_cache'] = raw_items
                    prog_bar.progress(50, text="Analyzing First 20 Stories...")
                    analyzed = process_ai_batch(raw_items[:20], KEY)
                    st.session_state['news_results'] = analyzed
                    st.session_state['news_offset'] = 20
                    prog_bar.progress(100, text="Done!"); time.sleep(0.5); prog_bar.empty(); st.rerun()
    ...
    if st.session_state['news_results']:
        for n in st.session_state['news_results']:
            ...
            st.markdown(f"""<div style="border-left: 4px solid {s_color}; padding-left: 10px; margin-bottom: 20px;"><div style="font-weight:bold; font-size:18px;"><span style="background:{s_color}; color:white; padding:2px 6px; border-radius:4px; font-size:12px;">{n.get('ticker','MKT')}</span> {n.get('summary', n.get('title'))}</div><div style="font-size:12px; color:#888; margin-top:4px;">{n.get('time','Recent')} &nbsp;|&nbsp; <a href="{n.get('link','#')}" style="color:#4dabf7; text-decoration:none;">Read Full Story ➤</a></div></div>""", unsafe_allow_html=True)

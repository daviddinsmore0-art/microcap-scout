                    ...
                    if api_key:
                        try:
                            ...
                            if "|" in ans:
                                ...
                                # INTERNAL LOGIC FIX: Match AI response to your s_map keys
...
def get_batch_data(tickers_list):
    ...
    try:
        ...
        for row in rows:
            ...
            # LOGIC FIX: Pass vol_pct to draw_card
...
if not st.session_state["logged_in"]:
    ...
    with st.sidebar:
        ...
        # --- API STATUS INDICATOR (NEW LOGIC) ---
        fh_on = "🟢" if st.secrets.get("FINNHUB_API_KEY") else "🔴"
        st.markdown(f"<div style='font-size:10px; color:#888;'>📡 Connection: Finnhub {fh_on}</div>", unsafe_allow_html=True)
    ...
    def render_dashboard():
        ...
        def draw_card(t, port_item=None):
            ...
            with st.container(border=True):
                ...
                # REVEALED VOLUME BAR (LOGIC FIX)
                st.markdown(f"<div class='metric-label'><span>Volume Status</span></div><div class='bar-bg'><div class='bar-fill' style='width:{d['vol_pct']}%; background:#00d4ff;'></div></div>", unsafe_allow_html=True)

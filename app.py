...
st.markdown("""
    <style>
        ...
        /* Cards WITH CLICK EFFECT (FIXED & UNIVERSAL) */
...
def execute_paper_trade(username, ticker, action, qty, price):
    ...
    bal = float(row[0]); cost = float(qty) * float(price)
...
def deactivate_stock(username, ticker, ptype):
    conn = get_connection(); cursor = conn.cursor()
    cursor.execute("SELECT shares, entry_price, realized_pl FROM user_portfolio WHERE username=%s AND ticker=%s AND portfolio_type=%s", (username, ticker, ptype))
    conn.execute("UPDATE user_portfolio SET is_active=FALSE WHERE username=%s AND ticker=%s AND portfolio_type=%s", (username, ticker, ptype))
...
def render_portfolio_row(row, data, token):
    ...
    html = f'<a href="?token={token}&ticker={row["ticker"]}" target="_self" style="text-decoration:none;"><div class="card" style="display:flex; justify-content:space-between; align-items:center; border-left:4px solid {color};"><div><div style="font-weight:bold; color:white;">{row["ticker"]}</div>{pl_html}</div><div style="text-align:right;"><div style="color:white; font-weight:bold;">${p:,.2f}</div><div style="color:{cc}; font-size:0.8rem;">{"▲" if ch>=0 else "▼"} {ch:.2f}%</div></div></div></a>'
...
def render_compact_watchlist(rows_list, current_token):
    ...
    for row in rows_list:
        ...
        h += f"<a href='?token={current_token}&ticker={row['ticker']}' target='_self' style='text-decoration:none;'><div class='scrolling-card card clickable-card' style='width: 140px;'><div style='font-weight:bold; font-size:0.95rem; color:white;'>{row['ticker']}</div><div style='font-size:0.65rem; color:#94a3b8; margin-top:5px;'>Risk: <span style='color:{color}'>{risk}</span></div></div></a>"
    h += '</div>'
...
def get_watchlist_header_date():
    return datetime.now(pytz.timezone('America/New_York')).strftime("%b %d")
...
if "token" not in st.query_params:
    ...
    with tab3:
        st.info("PIN reset available via support.")
    st.stop()

...
def init_db():
    try:
        ...
        """)
        # Added for persistent Morning Briefing
...
# --- SCANNER ENGINE (YOUR SCANNER LOGIC RESTORED + FINNHUB INJECT) ---
                    ...
                    if api_key:
                        try:
                            ...
                            if "|" in ans:
                                ...
                                # Internal fix for black bars
                                raw_s = parts[1].strip()
                                sentiment = "BULLISH" if "BULL" in raw_s else "BEARISH" if "BEAR" in raw_s else "NEUTRAL"
                        except: pass
    ...
    def render_dashboard():
        ...
        with t1:
            try:
                ...
                row = cursor.fetchone(); conn.close()
                if row: st.success(f"📌 **Morning Watchlist:** {', '.join(json.loads(row['picks']))}")
            except: pass
            cols = st.columns(3)

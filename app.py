...
# --- HYBRID ENGINE: BATCH PRICES + SURGICAL METADATA ---
def run_backend_update():
    """
    1. Batch Download: Fast prices/charts.
    2. Surgical Metadata: Fetches missing Earnings/Ratings safely.
    """
    try:
        ...
        existing = {row['ticker']: row for row in cursor.fetchall()}
        ...
        if to_fetch_price:
            ...
            for t in to_fetch_price:
                try:
                    ...
                    """
                    v = (t, final_price, day_change, rsi, vol_stat, trend, chart_json, pp_p, pp_pct,
                         final_price, day_change, rsi, vol_stat, trend, chart_json, pp_p, pp_pct)
                    cursor.execute(sql, v)
                    conn.commit()
                except: pass

        # 4. SURGICAL METADATA (Limit 3 per run)
        if to_fetch_meta:
                ...
                try:
                    ...
                    try:
                        ...
                        if isinstance(cal, dict) and 'Earnings Date' in cal:
                            ...
                            if future: e_val = min(future).strftime('%b %d')
                    except: pass
...
def get_pro_data(s):
    try:
        ...
        return {
            "p": price, "d": change, "name": display_name, "rsi": rsi_val, "vol_pct": vol_pct, "ai": "BULLISH" if trend == "UPTREND" else "BEARISH", "trend": trend, "pp": pp_html, "chart": chart_data,
...
if not st.session_state["logged_in"]:
    ...
    with st.sidebar:
        ...
        if st.button("⚡ Force Update"): run_backend_update(); st.rerun()
    ...
    def draw(t, port=None):
        ...
        st.markdown(f"<span class='info-pill' style='border-left: 3px solid {ai_col}'>AI: {d['ai']}</span> <span class='info-pill' style='border-left: 3px solid {tr_col}'>{d['trend']}</span> <span class='info-pill' style='border-left: 3px solid #333'>RATE: {d['rating']}</span>", unsafe_allow_html=True)
        st.markdown(f"<div class='metric-label'><span>RSI ({int(d['rsi'])})</span></div><div class='bar-bg'><div class='bar-fill' style='width:{d['rsi']}%; background:{b_col};'></div></div>", unsafe_allow_html=True)

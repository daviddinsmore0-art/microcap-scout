...
# --- SCANNER ENGINE (LOGIC FIX: FINNHUB INTEGRATION) ---
...
def run_gap_scanner(user_tickers, api_key):
    ...
    try:
        # Switching to Finnhub for the heartbeat scanner to avoid yFinance bans
        for t in scan_list:
            try:
                r = requests.get(f"https://finnhub.io/api/v1/quote?symbol={t}&token={fh_key}", timeout=5)
                data = r.json()
                if 'c' in data and data['c'] != 0:
                    curr_price = float(data['c'])
...
# --- NEWS ENGINE (LOGIC FIX: SENTIMENT VALIDATION) ---
                    ...
                    if api_key:
                        try:
                            ...
                            if "|" in ans:
                                ...
                                # Validation logic to fix black bars
                                raw_s = parts[1].strip()
...
if not st.session_state["logged_in"]:
    ...
    with st.sidebar:
        ...
        # --- API STATUS LIGHT (NEW LOGIC) ---
        fh_status = "🟢" if st.secrets.get("FINNHUB_API_KEY") else "🔴"
        oa_status = "🟢" if ACTIVE_KEY else "🔴"
        st.markdown(f"""
            <div style='font-size:10px; color:#888; margin-bottom:15px;'>
                📡 API Status: Finnhub {fh_status} | OpenAI {oa_status}
            </div>

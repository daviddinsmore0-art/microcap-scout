...
def init_db():
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS user_profiles (username VARCHAR(255) PRIMARY KEY, pin VARCHAR(50), display_name VARCHAR(100), email VARCHAR(255))")
...
def add_ticker_to_db(username, ticker):
    conn = get_connection(); cursor = conn.cursor()
    cursor.execute("INSERT IGNORE INTO user_portfolio (username, ticker) VALUES (%s,%s)", (username, ticker))
    ...
    if tab == "home":
        ...
        if port:
            if st.button("🔄 Refresh Data", key="ref_home"):
                with st.spinner("Scanning market..."): update_stock_data(port, username)
        ...
        with c1: new_t = st.text_input("Add Ticker", placeholder="e.g. AAPL", label_visibility="collapsed")
        with c2: 
            if st.button("Add", use_container_width=True): 
                if new_t: add_ticker_to_db(username, new_t.upper()); st.rerun()
        ...
        with st.expander("New Alert", expanded=True):
            t = st.selectbox("Ticker", get_user_portfolio(username) or [])

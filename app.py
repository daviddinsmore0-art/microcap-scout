...
# --- SCANNER ENGINE (FINNHUB LOGIC SWAP) ---
...
def run_gap_scanner(user_tickers, api_key):
    ...
    try:
        for t in scan_list:
            try:
                # Requests to Finnhub prevents yFinance blacklist
                ...
                if 'c' in data and data['c'] != 0:
                    ...
                    if abs(gap_pct) >= 1.0:
                        candidates.append({"ticker": t, "gap": gap_pct, "atr": 0.0, "price": curr_price})
                time.sleep(0.1) 
            except: continue
    except: return [] 
...
# --- NEWS ENGINE (LOGIC RESTORED + SENTIMENT FIX) ---
                    ...
                    if api_key:
                        try:
                            ...
                            if "|" in ans:
                                ...
                                # Internal fix to match CSS keys
...
if not st.session_state["logged_in"]:
    ...
    with st.sidebar:
        ...
        st.caption(f"📡 Connection: Finnhub {fh_on}")
